...
os.system('python main.py --edge_drop_prob 0.1 --dataset gowalla --if_pop 0 --cuda {} --model SGL-RW --if_tsne 0 --comment SGL_Origin-RW'.format(cuda))
#调整drop和add的概率超参数SGL5-RW
for drop in [0.05, 0.1, 0.2, 0.3, 0.4]:
    for add in [0.7, 0.8, 0.9, 1.0]:
        P_e_drop1, P_e_drop2 = drop, drop
        P_e_add1, P_e_add2 = add, add
        os.system('python main.py --edge_drop_prob 0.0 --dataset gowalla --if_pop 1 --pop_mode SGL5 --cuda {} --P_e_drop1 {} --P_e_add1 {} --P_e_drop2 {} --P_e_add2 {} --model SGL-RW --if_tsne 0 --comment SGL5_Adaptive_RW_HPS'.format(cuda, P_e_drop1, P_e_add1, P_e_drop2, P_e_add2))